...
| `integrations/pi/agentpod-errors` (extension) | `message_end` where `role === "assistant"` and `stopReason === "error"` (one per attempt, Pi's own retries included; `errorMessage` is `"<status> <json>"`), reported at `agent_settled`, which fires once after the last. No timer. Installed as the top-level file `~/.pi/agent/extensions/agentpod-errors.ts` (pi-acp's banner lists only top-level files); nothing to restart, since pi-acp starts a Pi per session. | `TurnErrorReport` keyed by `AGENTPOD_ACP_SESSION`, which the node sets on every adapter from acp.open's instance (the hub session id) and pi-acp passes on to Pi |
