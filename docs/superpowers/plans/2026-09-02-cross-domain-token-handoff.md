...
6. **Codes are single-use, enforced by one conditional UPDATE**, not read-then-write. Copy the shape of `redeemCredentialAuthorization` in `apps/hub/src/services/matrix-credential.ts`. Note: that behaviour was verified by hand when it was written but **no committed test pins it** — so copy the shape, and write your own concurrency test rather than trusting an existing one.
