...
> **Drift note — 17 September 2026.** This is the log of AgentPod's first seventeen days,
> kept as it was written. It describes a desktop app that no longer exists, and
> "production-ready" was that week's claim about that app, not a description of AgentPod now.
>
> There is no desktop app and no mobile one — the client is a web console — and the
> Tauri and SQLite architecture below has been replaced. AgentPod is now a
> console for **the machines agents already live on**, attaching to runtimes wherever they
> run. See [docs.agentpod.dev](https://docs.agentpod.dev) for what is true now, and
> [From a Sandbox to a Fleet](/blog/from-a-sandbox-to-a-fleet) for how the one became the
> other.
