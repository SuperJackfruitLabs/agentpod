...
import type { SkillInstallPlan, SkillInstallReceipt } from "@agentpod/contract";
...
export const skillOperations = pgTable(
  "skill_operations",
  {
    ...
    harness: text("harness").notNull(),
    profile: text("profile").notNull(),
    action: text("action").$type<"install" | "rollback">().notNull(),
    ...
    state: text("state")
      ...
      .notNull()
      .default("requested"),
    plan: jsonb("plan").$type<SkillInstallPlan>(),
    receipt: jsonb("receipt").$type<SkillInstallReceipt>(),
    error: text("error"),
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.id} ~ '^[a-f0-9]{32}$' AND ${t.action} IN ('install','rollback') AND ((${t.action}='install')=(${t.artifactId} IS NOT NULL))`,
    ),
    check(
